...
# 
# # Referencia: https://github.com/h2oai/h2o-3/blob/master/h2o-docs/src/product/tutorials/gbm/gbmTuning.Rmd
# 
# 
...
```{r modelini, message=FALSE, warning=FALSE}
...
```{r modeltwo, message=FALSE, warning=FALSE}
...
```{r modelgrid, message=FALSE, warning=FALSE}
gbm <- h2o.gbm(
...
```{r modelerror, message=FALSE, warning=FALSE}
...
```{r modelgriddeep, message=FALSE, warning=FALSE}
...
```{r modelgriddeeptwo, message=FALSE, warning=FALSE}
hyper_params = list( 
...
```{r loopone, message=FALSE, warning=FALSE}
for (i in 1:5) {
...
```{r modelloopeval, message=FALSE, warning=FALSE}
...
```{r modelupdate, message=FALSE, warning=FALSE}
...
```{r modellooptwo, message=FALSE, warning=FALSE}
for (i in 1:5) {
  gbm <- h2o.getModel(sortedGrid@model_ids[[i]])
...
```{r modellooptwoupdate, message=FALSE, warning=FALSE}
gbm <- h2o.getModel(sortedGrid@model_ids[[1]])
...
```{r message=FALSE, warning=FALSE}
h2o.saveModel(gbm, "./bestModel.csv", force = TRUE)
h2o.exportFile(preds, "./bestPreds.csv", force = TRUE)
```
...
```{r pojo, message=FALSE, warning=FALSE}
# Do not save. It takes very long and it is very big.
...
```{r modelensamble, message=FALSE, warning=FALSE}
prob = NULL
k = 10
for (i in 1:k) {
  gbm <- h2o.getModel(sortedGrid@model_ids[[i]])
...
```{r crossvalid, message=FALSE, warning=FALSE}
